...
export const CTASection: React.FC = () => {
  // REWORK NOTE: this used to flip to bg-white/text-black when the site was
  // in dark mode and vice-versa (a literal inversion of whatever theme the
  // visitor had chosen). That reads as accidental rather than intentional —
  // the "close" of the page would visually contradict every section before
  // it. This section now always renders the same deliberate dark, high
  // contrast treatment regardless of site theme, which is a more common and
  // more legible pattern for a closing CTA, and it reuses the Hero's
  // timeline-scrubber motif (very faint, in the background) so the page
  // opens and closes on the same idea.
  return (
    <section
      id="cta-section"
      className="section-x relative overflow-hidden py-32 text-center bg-[#0A0A0A] text-white"
    >
      {/* Echo of the Hero's signature scrubber line, very faint, as a
         full-width background texture rather than a literal control. */}
      <div className="absolute inset-x-0 top-1/2 -translate-y-1/2 h-px bg-gradient-to-r from-transparent via-[#4D65FF]/25 to-transparent pointer-events-none" />
      ...
      <div className="relative max-w-4xl mx-auto flex flex-col items-center">
        ...
        <motion.p
          ...
          className="text-base md:text-lg max-w-2xl mb-10 leading-relaxed text-zinc-400"
        >
          Book a call and we'll look at your offer and your audience, then <br className="hidden md:inline" />
          map out exactly what your channel and funnel could look like. You'll <br className="hidden md:inline" />
        ...
        {/* Bottom Trust Tags */}
        <p className="text-xs mt-8 tracking-wide text-zinc-600">
